...
export function createNativeBash(
  ...
  let state = isSandboxSessionState(run.sandboxState) ? run.sandboxState : null;
  ...
  async function recordState(next: SandboxSessionState, reset: boolean) {
    await db.transaction(async (tx) => {
      ...
      await tx
        ...
        .set({
          sandboxState: next,
          sandboxRef: String(next.providerState.sandboxId ?? next.providerState.containerId ?? ""),
          sandboxGeneration: generation,
        })
        .where(and(eq(runs.id, run.id), eq(runs.orgId, run.orgId)));
      await appendRunEvents(tx, run, [
        { type: "sandbox.created", payload: { provider: provider.backendId, generation } },
        ...(reset
          ? [
              {
  ...
  return async (id, command, files, metadata, signal, timeoutMs) => {
    ...
    if (!session && state) {
      ...
      if (session)
        await db.transaction(async (tx) => {
          await lockOwnedRun(tx, lease, run);
          await appendRunEvents(tx, run, [
    ...
    if (!session) {
      ...
      session = created;
      if (hadState) {
        const history = await listRunEvents(db, run.id, run.orgId);
    ...
    try {
      ...
      if (signal.aborted) {
        try {
          ...
          // Only a durable terminal decision authorizes deletion. Lease loss leaves the box
          // available to its new owner; duration cancellation is collected after finalization.
