...
// Acceptance and deduplication are session-level contracts; both engines must honour them.
const ENGINES = ["pi-executor", "pi-harness"] as const;
async function fixture(engine: (typeof ENGINES)[number]) {
  ...
  const options = {
    directory,
    engine,
    credential: () => ({ apiKey: "synthetic", baseUrl: upstream.url }),
  };
...
it.each(ENGINES)(
  "deduplicates concurrent, completed and restarted submissions without repeating model calls (%s)",
  async (engine) => {
...
it.each(ENGINES)(
  "rejects conflicting content under an accepted ID and scopes deduplication to the owning session (%s)",
  async (engine) => {
    const f = await fixture(engine);
    const first = await f.runtime.startTurn(f.session.id, request());
    await expect(
      f.runtime.startTurn(f.session.id, request({ prompt: "Different" })),
...
it.each(ENGINES)(
  "rolls acceptance, turn, events and workspace back on storage failure before dispatch (%s)",
  async (engine) => {
    const f = await fixture(engine);
...
it.each(ENGINES)(
  "does not resume interrupted work when a caller retries durable acceptance (%s)",
  async (engine) => {
    const f = await fixture(engine);
...
it.each(ENGINES)(
  "generates an acceptance ID that callers can reuse without changing legacy no-ID submissions (%s)",
  async (engine) => {
    const f = await fixture(engine);
    ...
    expect(
      await f.runtime.startTurn(f.session.id, { ...input, requestId: first.requestId }),
    ).toEqual(first);
    await finish(f.runtime, first.runId);
...
it.each(ENGINES)(
  "retries a legacy accepted request after upgrade without replaying its model call (%s)",
  async (engine) => {
    const f = await fixture(engine);
    const first = await f.runtime.startTurn(f.session.id, request());
    await finish(f.runtime, first.runId);
